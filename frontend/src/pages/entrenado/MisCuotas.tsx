...
const estadoConfig: Record<string, { label: string; bg: string; text: string }> = {
  pagado: { label: 'Pagada', bg: 'bg-green-100 dark:bg-green-900/50', text: 'text-green-700' },
  pendiente: { label: 'Pendiente', bg: 'bg-yellow-100 dark:bg-yellow-900/50', text: 'text-yellow-700' },
  vencido: { label: 'Vencida', bg: 'bg-orange-100 dark:bg-orange-900/50', text: 'text-orange-700' },
  mora: { label: 'En mora', bg: 'bg-red-100 dark:bg-red-900/50', text: 'text-red-700' },
};
    ...
    <div className="space-y-6 fade-in">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Cuota actual</h2>
            ...
            {cuotaActual.pagos && cuotaActual.pagos.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Pagos registrados</h3>
                <div className="space-y-2">
                  {cuotaActual.pagos.map((pago) => (
                    ...
                    >
                      ...
                      {pago.comprobante && (
                        <span className="text-xs text-gray-400 dark:text-gray-500">#{pago.comprobante}</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          {/* Mensaje de estado */}
          {cuotaActual.estado === 'pagado' && (
            <div className="px-6 py-4 bg-green-50 dark:bg-green-900/30 border-t border-green-100 dark:border-green-800">
              <div className="flex items-center gap-3">
          ...
          {(cuotaActual.estado === 'vencido' || cuotaActual.estado === 'mora') && (
            <div className="px-6 py-4 bg-red-50 dark:bg-red-900/30 border-t border-red-100 dark:border-red-800">
              <div className="flex items-center gap-3">
                <svg className="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <p className="text-red-800">Tu cuota está vencida. Contactá a tu entrenador para renovar.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {!cuotaActual && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">No hay cuotas registradas</h3>
          <p className="text-gray-500 dark:text-gray-400">Tu entrenador registrará tu cuota cuando corresponda</p>
        </div>
      )}
      ...
      {historial.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Historial de cuotas</h2>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {historial.map((cuota) => (
              <div key={cuota.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      cuota.estado === 'pagado' ? 'bg-green-100 dark:bg-green-900/50' : 'bg-gray-100 dark:bg-gray-700'
                    }`}>
                      {cuota.estado === 'pagado' ? (
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">{cuota.plan.nombre}</p>
      ...
      {cuotas && cuotas.length > 0 && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500 dark:text-gray-400">Total pagado históricamente</span>
