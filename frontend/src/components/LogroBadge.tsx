...
export default function LogroBadge({
  nombre,
  descripcion,
  icono,
  color,
  ...
  const iconPath = iconMap[icono] || iconMap.star;
    ...
    <div className="group relative flex flex-col items-center">
      ...
      >
        <svg
          ...
          fill="none"
          stroke={desbloqueado ? color : '#9ca3af'}
          viewBox="0 0 24 24"
        >
          {iconPath}
        </svg>
      ...
      {showTooltip && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-50">
          <div className="bg-gray-900 text-white text-xs rounded-lg py-2 px-3 whitespace-nowrap shadow-lg">
            ...
            <p className="text-gray-300 text-xs">{descripcion}</p>
            {desbloqueado_en && (
              <p className="text-green-400 text-xs mt-1">
                Desbloqueado el{' '}
                {new Date(desbloqueado_en).toLocaleDateString('es-AR', {
                  day: 'numeric',
                  month: 'short',
                })}
              </p>
      ...
      {size !== 'sm' && (
        <p
          className={`mt-1 text-xs text-center ${
            desbloqueado ? 'text-gray-700 font-medium' : 'text-gray-400'
          }`}
        >
          {nombre}
        </p>
